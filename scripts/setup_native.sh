...
apt-get install -y --no-install-recommends \
    ...
    git python3 python3-pip ccache \
...
pip3 install --no-cache-dir -r "$REPO_ROOT/llvm-apr-benchmark/requirements.txt"
...
cat > "$ENV_FILE" <<EOF
# Generated by scripts/setup_native.sh. Source this in every shell before
...
echo
echo "== done =="
echo "Environment written to $ENV_FILE -- source it before running anything:"
